...
const sendErrorResponse = async (req, res, code, error) => {
    try {
        ...
        const message = (error && error.message) ? error.message.replace(/[^a-zA-Z ]/g, "") : STATUS_MESSAGE.INTERNAL_SERVER_ERROR;
...
module.exports = {sendUnauthorizedErrorResponse, sendErrorResponse, sendResponse}