...
const getCricketersDetails = (req, response)=>{
    ...
    client.query('select * from cricketerss', (err, res)=>{
        if(err){
...
const onInsert = (req, response) => {
    ...
    response.type('json')
    client.query(`insert into cricketers(id, name, age, dob)
    values(${id}, '${name}', ${age}, '${dob}')`,(err, res)=>{
        let result = ""
        if(err){
...
const onInsertUsingFunction =  (req, response) => {
    ...
    response.type('json')
    client.query(`select onHandleInsert('${name}', ${age}, '${dob}')`,(err, res)=>{
        let result = ""
        if(err){
            result = "API Request Failed"
            console.log("insert error", err);
...
const onHandleDelete = (request, response)=>{
    ...
    client.query(`delete from cricketers where id = ${request.params.id}`,(err, res)=>{
        let result = ""
        if(err){
            result = "API Request Failed"
...
const onHandleUpdate =  (request, response) => {
    client.query(`update cricketers set name = '${request.body.name}' where id = ${request.body.id}`, (err, res) => {
        let result = ""
        if(err){
            result = "API Request Failed"
...
const onHandleUpdateUsingFunction = (request, response) => {
    console.log("result099", `${request.body.id}, ${request.body.name||null}, ${request.body.dob||null}`);
    client.query(`select onUpdate(${request.body.id}, ${request.body.name||null}, '${request.body.dob||null}')`, (err, res) => {
        let result = ""
        if(err){
            result = "API Request Failed"
            console.log("update error", err);
...
const getCricketer = (request, response) => {
    ...
    client.query(`select * from cricketers where id = ${request.params.id}`,(err, res)=>{
        let result = ""
        if(err){
            result = "API Request Failed"
...
const onBulkInsert = (request, response)=>{
    ...
    client.query(`select bulk_insert('${JSON.stringify(request.body)}')`, (err, res)=>{
        let result = ""
        if(err){
...
module.exports = {getCricketersDetails, onBulkInsert, getCricketer, onHandleUpdateUsingFunction, onHandleUpdate, onHandleDeleteUsingProcedure, onHandleDelete, onInsertUsingFunction, onInsert }